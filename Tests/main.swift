...
check(changelog.contains("## v1.2.1"), "Changelog must document current app version")
check(info.contains("<key>CFBundleShortVersionString</key><string>1.2.1</string>"), "Info.plist version must match current changelog entry")
let ci = readText(".github/workflows/ci.yml")
check(ci.contains("branches: [main]"), "CI push trigger must explicitly include main")
check(ci.contains("tags: [\"v*\"]"), "CI push trigger must explicitly include v* release tags")
print("PASS: repository metadata, release-history links, license link, CI workflow, version parity, tag CI")
